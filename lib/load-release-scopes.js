...
module.exports = ({releaseScopes}, {cwd}) => {
  ...
  if (releaseScopes) {
    ...
    // LoadedReleaseScopes.forEach((rule) => {
    ...
    //   } else if (!RELEASE_SCOPES.includes(rule.release) && rule.release !== null && rule.release !== false) {
    //     throw new Error(
    //       `Error in commit-analyzer configuration: "${
    //         rule.release
    //       }" is not a valid release type. Valid values are: ${JSON.stringify(RELEASE_SCOPES)}`
    //     );
    //   }
    // });
  } else {
